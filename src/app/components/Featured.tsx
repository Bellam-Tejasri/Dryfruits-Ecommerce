          ...
          <div key={cat.id} className="px-3">
            <div className="rounded-lg w-auto h-auto shadow-md overflow-hidden">
              ...
              <Link href={`/collections/${cat.slug}`}>
                <div className="flex items-center justify-center p-6 w-full h-auto cursor-pointer hover:scale-105 transition">
                  <Image
                    src={cat.img}
                    alt={cat.name}
                    width={800}
              ...
              {/* Product Info */}
              <div className="bg-white p-6 flex flex-col text-black">
                <div className="flex justify-between items-center">
                  ...
                  <p className="flex items-center text-sm gap-2">
                    <span className=" p-1 flex items-center justify-center">
                      <Calendar size={12} className="text-[#6D4C41]" />
                    </span>
                    {cat.name}
                  </p>
                </div>
                ...
                {/* View More button */}
                <Link href={`/collections/${cat.slug}`}>
                  <button className="mt-4 border border-[#6D4C41] text-[#6D4C41] px-4 py-1 rounded-full hover:bg-[#6D4C41] hover:text-white transition">
                    View more
                  </button>
                </Link>
              </div>
